...
export class MemoryGovernanceService {
  ...
  async getStats(): Promise<MemoryGovernanceStats> {
    const rawMessageGroups = await this.prisma.$queryRaw<
      { purposeTag: string; count: bigint }[]
    >`SELECT "purposeTag", COUNT(*) as count FROM "conversation_messages" GROUP BY "purposeTag"`;
    const rawInteractionGroups = await this.prisma.$queryRaw<
      { purposeTag: string; count: bigint }[]
    >`SELECT "purposeTag", COUNT(*) as count FROM "character_interactions" GROUP BY "purposeTag"`;
    const messageGroups = rawMessageGroups.map((row) => ({
      purposeTag: row.purposeTag,
      _count: { _all: Number(row.count) },
    }));
    const interactionGroups = rawInteractionGroups.map((row) => ({
      purposeTag: row.purposeTag,
      _count: { _all: Number(row.count) },
    }));
    const [expiredMessageGroups, expiredInteractionGroups] =
      await Promise.all([
