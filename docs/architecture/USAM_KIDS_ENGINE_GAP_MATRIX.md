...
| 2 | Knowledge Graph Engine | `Concept`, `ConceptPrerequisite`, `CompetencyPrerequisite`, `Domain/Skill/Competency/LearningObjective` tables exist; NO traversal/graph query service found anywhere in `backend/src` | Missing | Data model has the right shape (prerequisite edges) but there is no service that walks the graph (e.g. "what's the shortest unlock path to X", cycle detection, prerequisite closure). Build custom on top of existing Postgres tables — do not adopt Neo4j yet, current scale doesn't need a graph DB (see inventory's own "don't over-engineer" guidance). |
