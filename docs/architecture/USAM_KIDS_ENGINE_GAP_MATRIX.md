...
| Economy Engine | `Progression.coins Int` field exists as a raw counter incremented alongside XP (progression.service.ts), but there is **no shop, no spending, no item/currency-sink model, no `Item`/`Store`/`Purchase` table in schema.prisma**, and no frontend page for spending coins. | Partially implemented | Coins are earned but have nothing to be spent on — a currency with no economy around it. Needs a real sink (shop/cosmetics/unlocks) to count as an actual Economy Engine. |
...
| Engine | Real code found | Classification | Notes |
|---|---|---|---|
| Entrepreneurship Engine | `EntrepreneurshipConcept` model exists in schema.prisma (name/slug/description/category/ageAppropriate/order) and was seeded with real per-age-band content by `backend/prisma/seeds/seed-cross-curricular.ts` (commit `e25da26`, prior wave). **No backend module/controller exposes this model** — `grep -rn EntrepreneurshipConcept backend/src` matches only the seed file. No frontend reference anywhere in `frontend/src/`. `CharacterRole.ENTREPRENEURSHIP_MENTOR` enum value exists but no service/route implements it. | Partially implemented | Data model + real seed content exist; zero API surface and zero UI. A learner cannot reach this content today even if seeded rows are live in the DB — this job could not confirm live row counts (no DB/SSH access from this environment), but the absence of any controller route makes the point moot: seeded or not, it's unreachable. |
| Financial Literacy Engine | Same pattern as above: `FinancialLiteracyConcept` model + seed data in `seed-cross-curricular.ts`, **no controller/module/route**, no frontend page. | Partially implemented | Identical gap to Entrepreneurship Engine — schema + seed content only, no delivery mechanism. |
...
| AI Literacy Engine | `AILiteracyConcept` model in schema.prisma (ageAppropriate/category/order fields), seeded with real content by `seed-cross-curricular.ts` (same commit as Entrepreneurship/Financial above — includes "What Is AI", "Narrow vs General AI", "Learning From Examples" etc., verified by reading the seed file directly, not just its filename). **No controller/module exposes it** — same `grep` result as the other two Concept models: zero hits outside the seed file. No frontend page. | Partially implemented | Best-seeded of the three Concept models content-wise (age-banded, pedagogically real), but equally unreachable via any API — schema + content only, no delivery layer. |
...
| Engine | Real code found | Classification | Notes |
|---|---|---|---|
| Learning Analytics Engine | Real: `LearningEvent` model (schema.prisma) with 18-value `LearningEventType` enum (ACTIVITY_STARTED/COMPLETED, MASTERY_CHANGED, ACHIEVEMENT_EARNED, etc.); `backend/src/modules/learning/services/learning-event.service.ts` (`recordEvent`, `recordActivityStarted`, `recordActivityCompleted`, plus stats/pattern methods); controller routes `/learning/events`, `/learning/events/stats`, `/learning/events/recent`, `/learning/events/patterns` (learning.controller.ts). No dedicated frontend analytics dashboard found (`grep -l analytics frontend/src` — none) beyond the gamification progress page. | Partially implemented | Backend event-logging + stats pipeline is real and reasonably complete; no learner/parent-facing analytics UI consumes it yet (parents.service.ts has some read access but no dedicated dashboard page located). |
