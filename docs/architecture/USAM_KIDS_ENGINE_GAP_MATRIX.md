...
| Career Exploration Engine | No model, no service, no seed, no frontend reference. `CharacterRole` enum has no career-guide role either. | Missing | Zero trace anywhere in the codebase. |
...
| Metacognition Engine | No model, no service, no seed, no frontend trace anywhere in backend or frontend. `LearningEventType` has no reflection/self-assessment event types. | Missing | Zero implementation; would require new model (e.g., learner self-ratings, reflection prompts) and UI. |
...
| Arabic Educational Content Engine | No model, no service specific to controlled (non-hallucinated) Arabic curriculum content. `Translation` model (schema.prisma:985) supports storing `ar`/`ar-EG` values per entity/field but is a generic string-value store, not a curated "official curriculum" content system with any human-approval gate. | Missing | The generic `Translation` table could theoretically host Arabic educational strings, but nothing enforces "controlled, not AI-hallucinated" — no approval workflow, no source-attribution field, no distinction between AI-generated and human-authored translation rows. |
| Translation Engine | `Translation` model (schema.prisma:985: `entityType`/`entityId`/`field`/`language`/`value`, unique per entityType+entityId+field+language) + `TranslationService` (`backend/src/modules/learning/services/translation.service.ts`, 351 lines per Part 5's confirmed read) with real `upsertTranslation`/`getTranslatedEntity` CRUD, supporting `en`/`ar`/`ar-EG` as the `SupportedLanguage` type. **UPDATE (localization-wave-1)**: `TranslationController` (`backend/src/modules/learning/translation.controller.ts`) now exposes it — `GET /translations/languages`, `GET /translations/:entityType/:entityId[?language=]`, `GET /translations/:entityType/:entityId/:field?language=`, `POST /translations`, `POST /translations/batch` — registered in `LearningModule`, live-verified via curl against production. | Partially implemented | The HTTP-surface gap flagged in the original pass is fixed: the service is reachable now. Still Partially implemented rather than fully done because no learning-content entities (missions/activities/concepts) actually have `Translation` rows populated yet — the plumbing works end-to-end but there's no real bilingual curriculum content flowing through it. That population is a content-authoring task, not a code gap. |
| Pronunciation Accent Engine | No trace — this is a sub-specialization of the already-Missing Pronunciation Engine (English Learning section above); no accent-specific IPA/audio-comparison logic exists at any level. | Missing | Nothing to partially credit; the parent capability doesn't exist either. |
| Voice Emotion/Prosody Engine | No trace anywhere in `voice.service.ts` or `character.service.ts` — the `mood` field on `CharacterInteraction`/`CharacterResponse` (Part 8's Character Intelligence row above) is an LLM-generated text label describing the *character's* expressed mood in a reply, not a signal extracted from the *learner's* voice prosody. The Voice Pipeline (voice module) only carries transcript text between ASR and the LLM — no prosodic/emotional feature extraction on the input audio at any stage. | Missing | Confirms the inventory's own caution ("don't overclaim emotion-reading accuracy from voice without validation") is moot here since no such extraction exists to overclaim about. |
...
| Asset Management Engine | No asset/license/attribution-per-asset tracking model or service found — `AvatarCosmetic` model (schema.prisma:1206, used by the real Gamification cosmetic-shop feature per commit `fe7e6d5`) has no `license`/`source`/`attribution` fields, and no other model in the schema carries per-asset provenance metadata. | Missing | This directly matters given the inventory's own License Registry mandate (Part 3, already tracked separately in this file for whole *libraries* like Pyodide/Sandpack/LanguageTool/Presidio) — but there is no equivalent per-*asset* (image/audio/font) tracking, which is a distinct, narrower requirement that remains unaddressed. |
...
| Flashcard Engine | Zero trace — `grep -rli flashcard backend/src frontend/src` returns no hits anywhere in the codebase. | Missing | Nothing built; `EnglishStrand`/`Activity` could theoretically host flashcard-style content but no flashcard-specific interaction model or spaced-presentation UI exists. |
...
| Observability Engine | Zero trace — no OpenTelemetry/Prometheus/Grafana/Sentry SDK or config found in `backend/package.json`'s implied dependency surface (no import statements for any of these found via repeated greps across this and prior passes) or `backend/src/main.ts`. Only `console.log` (`main.ts`'s bootstrap banner) and NestJS's built-in `Logger` class (used extensively, e.g. `character.service.ts`, `moderation.service.ts`) exist for observability. | Missing | Consistent with this doc's own prior note (Part 1, row-adjacent Reconciliation section) that this "overlaps directly with Dept 3 DevOps/SRE agents" — genuinely nothing beyond basic structured logging exists yet; no metrics, traces, or error-aggregation service. |
...
| Localization CMS | Already logged Missing as part of Localization Engine (Part 8) — no separate authoring-tool trace found either; would require both the (missing) CMS above and the (missing) Localization Engine to exist first. | Missing | Compound dependency on two already-Missing systems — not a new independent finding. |
| Translation QA Engine | Already logged Missing/Partially-implemented context — `TranslationService` (Part 8, Localization section: real CRUD, zero controller/route) has no quality-review layer of any kind (no reviewer-approval field on the `Translation` model, no flagging workflow). | Missing | Even if Translation Engine's routing gap were fixed, there is still no QA layer sitting on top of it — a compound but distinct gap from the routing issue already logged. |
